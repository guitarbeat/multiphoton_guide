        ...
        if table_info_response and table_info_response.data:
            if len(table_info_response.data) > 0:
                ...
                for col in df_copy.columns:
                    if col in db_column_mapping and db_column_mapping[col] in available_columns:
                ...
                mapped_columns = set(
                    db_column_mapping.get(col, col) for col in df_copy.columns
                )
                if (
                    columns_to_drop := mapped_columns
                    - available_columns
                    - {'id'}
                ):
                    for col in df_copy.columns[:]:
                ...
                if column_renames := {
                    col: db_column_mapping.get(col, col)
                    for col in df_copy.columns
