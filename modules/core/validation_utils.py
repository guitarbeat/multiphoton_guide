...
from .constants import GDD_RANGE, POWER_RANGE, WAVELENGTH_RANGE
...
class ValidationResult:
    ...
    def __init__(
        self, is_valid: bool, message: str = "", field_name: str = "", value: Any = None
    ):
    ...
    try:
        ...
        if np.isnan(numeric_value):
            return ValidationResult(
                False, f"{field_name} cannot be NaN", field_name, value
            )

        # Check for zero if not allowed
        if not allow_zero and numeric_value == 0:
            return ValidationResult(
                False, f"{field_name} must be greater than 0", field_name, value
            )

        # Check minimum value
        if min_value is not None and numeric_value < min_value:
            return ValidationResult(
                False, f"{field_name} must be at least {min_value}", field_name, value
            )

        # Check maximum value
        if max_value is not None and numeric_value > max_value:
            return ValidationResult(
                False, f"{field_name} must not exceed {max_value}", field_name, value
            )

        return ValidationResult(
            True, f"{field_name} is valid", field_name, numeric_value
        )

    except (ValueError, TypeError):
        return ValidationResult(
            False, f"{field_name} must be a valid number", field_name, value
        )
...
def validate_form_data(
    form_data: Dict[str, Any], field_configs: Dict[str, Dict]
    ...
    for field_name, field_config in field_configs.items():
        ...
        #     results[field_name] = validate_wavelength(value)
            ...
            results[field_name] = ValidationResult(
                True, f"{field_name} passed validation", field_name, value
            )
...
def display_validation_message(
    result: ValidationResult, message_type: str = None
) -> None:
...
def display_form_validation_results(
    results: Dict[str, ValidationResult], show_success: bool = False
) -> bool:
