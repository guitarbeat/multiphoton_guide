...
def create_default_dataframe(df_type, additional_columns=None):
    ...
    if df_type not in COLUMN_SCHEMAS:
        raise ValueError(f"Unknown DataFrame type: {df_type}. Available types: {list(COLUMN_SCHEMAS.keys())}")
...
def load_measurement_dataframe(df_type):
    ...
    if df_type not in FILE_MAPPINGS:
        raise ValueError(f"Unknown DataFrame type: {df_type}. Available types: {list(FILE_MAPPINGS.keys())}")
...
def create_two_column_layout(left_function, right_function, layout_type="theory_practice"):
    ...
    if layout_type not in LAYOUT_RATIOS:
        raise ValueError(f"Unknown layout type: {layout_type}. Available types: {list(LAYOUT_RATIOS.keys())}")
