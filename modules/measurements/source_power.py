...
from modules.core.constants import SOURCE_POWER_FILE, SOURCE_POWER_COLUMNS
...
def render_source_power_form():
    ...
    with col1:
        pump_current = st.number_input(
            "Pump Current (mA):", 
            min_value=0, max_value=8000, value=2000, step=250,
    ...
    if not source_power_df.empty:
        ax.scatter(source_power_df["Pump Current (mA)"], source_power_df["Measured Power (W)"],
                   color='#4BA3C4', s=50, alpha=0.7, label='Previous Measurements')
    # Plot current (unsaved) value if entered
    if pump_current and power > 0:
        ax.scatter([pump_current], [power], color='red', s=100, label='Current Entry', zorder=10)
...
def render_source_power_visualization():
    ...
    # Display metrics
    create_metric_row([
        {"label": "Latest Power", "value": f"{filtered_df['Measured Power (W)'].iloc[-1]:.2f} W"},
        {"label": "Pump Current", "value": f"{filtered_df['Pump Current (mA)'].iloc[-1]} mA"},
        {"label": "Power Ratio", "value": f"{filtered_df['Measured Power (W)'].iloc[-1] / filtered_df['Expected Power (W)'].iloc[-1]:.2f}"}
    ])
    ...
    def plot_power_vs_current(fig, ax):
        ...
        ax.plot(expected_x, expected_y, color='#BF5701', linestyle='--', 
