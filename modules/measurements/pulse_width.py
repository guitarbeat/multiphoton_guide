...
def render_pulse_width_theory_and_procedure():
    ...
    with tab2:
        ...
        st.warning("**CRITICAL:** Optimal GDD values are wavelength-dependent. Repeat this procedure for each wavelength you commonly use.")
        st.warning("**CRITICAL:** Keep the laser power constant throughout the optimization process.")
        
        # Display only the pixel histogram image which we know exists
        st.image(get_image_path("pixel_histogram.png"), caption="The pixel intensity value distribution is shown for one image frame while live scanning. The spread of this distribution is used to optimize laser pulse width.")
...
def render_pulse_width_visualization():
    ...
    def plot_gdd_optimization_example(fig, ax):
        ...
        # Plot data points
        ax.plot(x, y1, 'o-', color='#4BA3C4', linewidth=2, markersize=8, label='Mean Pixel Value')
        
        # Mark optimal GDD
        optimal_gdd = 12000
        ax.axvline(x=optimal_gdd, color='#BF5701', linestyle='--', alpha=0.7, 
...
def render_pulse_width_tips():
    ...
    with st.expander("🔍 Histogram Analysis", expanded=True):
        ...
        st.image(get_image_path("pixel_histogram.png"), caption="Example pixel histogram showing intensity distribution during live scanning")
