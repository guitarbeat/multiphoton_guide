...
def render_laser_power_tab(use_sidebar_values=False):
    ...
    sample_tab, source_tab = st.tabs(["Laser Power at the Sample", "Laser Power at the Source"])
        ...
        with recent_tab:
            ...
            if not laser_power_df.empty:
                ...
                if not filtered_df.empty:
                    ...
                    st.dataframe(
                        filtered_df[["Date", "Modulation (%)", "Measured Power (mW)", "Measurement Mode", "Sensor Model"]],
                        hide_index=True,
    ...
    with source_tab:
        ...
        tab_recent, tab_theory = st.tabs([
            "Recent Measurements", "Theory & Procedure"
        ])
        ...
        with tab_recent:
            ...
            if not source_power_df.empty:
                ...
                if not filtered_df.empty:
                    filtered_df = filtered_df.sort_values("Date", ascending=False).head(5)
                    st.dataframe(
                        filtered_df[["Date", "Pump Current (mA)", "Measured Power (W)", "Expected Power (W)", "Notes"]],
                        hide_index=True,
                        use_container_width=True
                    )
        with tab_theory:
    ...
    with st.form(key="quick_measurement_form"):
        ...
        with col2:
            ...
            measurement_mode = st.radio(
                "Measurement Mode:", 
                MEASUREMENT_MODES,
                index=0 if st.session_state.get('measurement_mode', 'Stationary') == "Stationary" else 1,
        ...
        with col3:
            ...
            fill_fraction = st.session_state.get('fill_fraction', 100)  # Default from session state
