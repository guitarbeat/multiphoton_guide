...
class TestPDFViewer:
    ...
    def setup_method(self):
        ...
        self.test_pdf_path = Path(__file__).parent.parent / "assets" / "s41596-024-01120-w.pdf"
    
    def test_pdf_path_exists(self):
        """Test that the PDF file exists in the expected location."""
        assert self.test_pdf_path.exists(), f"PDF file not found at {self.test_pdf_path}"
        assert self.test_pdf_path.is_file(), f"PDF path is not a file: {self.test_pdf_path}"
        ...
        with patch('streamlit.columns') as mock_columns, \
            ...
            with patch.object(st, 'session_state', {}):
                ...
                try:
                    ...
                    if "annotations must be a list of dictionaries" in str(e):
                        pytest.fail(f"render_reference_content still has annotations error: {e}")
                    else:
                        raise e
