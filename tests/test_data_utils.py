...
from modules.core.data_utils import (  # noqa: E402
    load_dataframe,
    save_dataframe,
    ensure_columns,
...
def test_safe_numeric_conversion(test_dataframe):
    ...
    assert (
        result["Wavelength (nm)"].dtype.kind in "iuf"
    )  # integer, unsigned int, or float
...
def test_safe_numeric_conversion_edge_cases(test_dataframe):
    ...
    result = safe_numeric_conversion(
        problematic_df, ["Wavelength (nm)", "Modulation (%)", "Measured Power (mW)"]
    )
